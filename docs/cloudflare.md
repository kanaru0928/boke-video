...
GoバックエンドはCloudflare Accessが付与する`Cf-Access-Jwt-Assertion`を検証します。OBS入力はこの経路に含めません。OBSのRTMP接続ではCloudflare Access用のHTTPヘッダーを送れないため、OBS入力はMediaMTXのRTMP認証で守ります。
