...
このdocsの`example.com`はプレースホルダーです。実サーバーへ配置するときだけ、環境変数、Caddyfile、Cloudflare Tunnel設定、OvenMediaEngine設定で実ドメインへ置き換えます。実ドメイン、AccessのAUD tag、署名鍵、API tokenを含む`.env`実体はgitへ入れません。
...
| ファイル | 用途 |
| --- | --- |
| `deploy/cloudflared/cloudflared.env.example` | cloudflared token環境変数例 |
| `deploy/backend/backend.env.example` | Goバックエンド環境変数例 |
| `deploy/caddy/Caddyfile.example` | OracleのTLS入口設定例 |
| `deploy/caddy/caddy.env.example` | Caddyホスト名環境変数例 |
| `deploy/ovenmediaengine/Server.xml.example` | OvenMediaEngine設定例 |
| `deploy/systemd/*.service` | systemdユニット例 |
| `frontend/.env.production.example` | フロントエンドビルド環境変数例 |
...
sudo install -m 600 deploy/backend/backend.env.example /etc/boke-video/backend.env
sudo install -m 600 deploy/caddy/caddy.env.example /etc/boke-video/caddy.env
sudo install -m 600 deploy/cloudflared/cloudflared.env.example /etc/boke-video/cloudflared.env
